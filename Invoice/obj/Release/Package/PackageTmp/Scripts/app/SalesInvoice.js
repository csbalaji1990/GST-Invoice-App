$(document).ready(function () {
    ...
    function createDbItemTable() {
        Inventory.transaction(function (transaction) {
            ...
            transaction.executeSql("CREATE TABLE DbItem(ItemCode TEXT NOT NULL,ItemName TEXT NOT NULL,ItemDescription TEXT NOT NULL,HsnSac TEXT NOT NULL,Price DECIMAL NOT NULL,InclusiveTax TEXT NOT NULL,UnitId INT NOT NULL,TaxRate DECIMAL NOT NULL,Cess DECIMAL NOT NULL,StockQuantity DECIMAL NOT NULL)", []);

            for (var i = 0; i < itemList.length; i++) {
                transaction.executeSql("INSERT INTO DbItem(ItemCode,ItemName,ItemDescription,HsnSac,Price,InclusiveTax,UnitId,TaxRate,Cess,StockQuantity) VALUES(?,?,?,?,?,?,?,?,?,?)", [itemList[i].ItemCode, itemList[i].ItemName, itemList[i].ItemDescription, itemList[i].HsnSac, itemList[i].Price, itemList[i].InclusiveTax, itemList[i].UnitId, itemList[i].TaxRate, itemList[i].Cess, itemList[i].StockQuantity]);
            }
        });
    }
    ...
    function addRow() {
        $.ajax({
            url: "../../Content/SalesInvoice_Item_Template.txt",
            dataType: "text",
        ...
        $.each(itemDetails, function (i, itemDetail) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $("#tblItems .rowItem:nth-child(" + (i + 1) + ") .lblStockQuantity").html(itemDetail.StockQuantity);
    ...
    $("#tblItems").on("change", ".txtItemCode", function () {
        ...
        Inventory.transaction(function (tx) {
            tx.executeSql("SELECT ItemName,ItemDescription,HsnSac,Price,InclusiveTax,UnitId,TaxRate,Cess,StockQuantity FROM DbItem where ItemCode=?", [txtItemCode.val()], function (tx, results) {
                if (results.rows.length > 0) {
                    ...
                    $(lblStockQuantity).html(results.rows.item(0).StockQuantity);
    ...
    $(document).on("submit", "#formSalesInvoice", function () {
        ...
        var model = {
            ...
            Total: $("#Total").html(),
            Terms: $("#Terms").val(),
            SalesInvoiceItemDetails: itemDetails,
        };
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success === true) {
                    window.open("/Sale/PrintInvoiceTemplate1?InvoiceNo=" + data.InvoiceNo, "_blank");
                    window.location = "/Sale/InvoiceList";
